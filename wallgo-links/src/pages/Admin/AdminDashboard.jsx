...
const StatusBadge = ({ status }) => {
  const map = { 
    ...
    User: { bg: 'var(--bg-alt)', text: 'var(--text-muted)' }
  };
...
const OverviewSection = () => {
    ...
    const cards = [
      { label: 'Total Users', value: stats?.totalUsers || 0, color: '#7158E2', icon: Users, desc: 'Registered Publishers' },
      { label: 'Total Links', value: stats?.totalLinks || 0, color: '#3b82f6', icon: Globe, desc: 'Active Aliases' },
      { label: 'Global Traffic', value: (stats?.totalClicks || 0).toLocaleString(), color: '#10b981', icon: BarChart2, desc: 'Verified Impressions' },
      { label: 'Platform Revenue', value: `$${stats?.totalEarnings || '0.00'}`, color: '#f59e0b', icon: DollarSign, desc: 'Gross Monetization' },
      { label: 'Total Payouts', value: `$${stats?.totalPayouts || '0.00'}`, color: '#10b981', icon: CheckCircle, desc: 'Settled Requests' },
      { label: 'Pending Audit', value: stats?.pendingPayouts || 0, color: '#ef4444', icon: Clock, desc: 'Action Required' },
    ];
  
    return (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
        {cards.map((c, i) => (
          <div key={i} style={{ background: 'var(--bg-card)', padding: '1.5rem', borderRadius: '24px', border: '1px solid var(--border)', transition: '0.3s' }}>
                    ...
                    <tbody>
                        {users.map(u => (
                            <tr key={u._id} style={{ borderBottom: '1px solid var(--border)' }}>
                                ...
                                <td style={{ padding: '1rem 1.5rem', fontWeight: '800', color: '#10b981' }}>${(u.balance || 0).toFixed(2)}</td>
...
const PayoutsSection = () => {
    ...
    return (
        <div style={{ background: 'var(--bg-card)', borderRadius: '24px', border: '1px solid var(--border)', overflow: 'hidden' }}>
            <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border)' }}>
                <h3 style={{ fontSize: '1.25rem', fontWeight: '900' }}>Withdrawal Requests</h3>
            </div>
            <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr style={{ background: 'var(--bg-alt)' }}>
                            <th style={{ textAlign: 'left', padding: '1rem 1.5rem', color: 'var(--text-light)', fontSize: '0.75rem', fontWeight: '800' }}>USER</th>
                            <th style={{ textAlign: 'left', padding: '1rem 1.5rem', color: 'var(--text-light)', fontSize: '0.75rem', fontWeight: '800' }}>AMOUNT</th>
                            <th style={{ textAlign: 'left', padding: '1rem 1.5rem', color: 'var(--text-light)', fontSize: '0.75rem', fontWeight: '800' }}>METHOD</th>
                            <th style={{ textAlign: 'left', padding: '1rem 1.5rem', color: 'var(--text-light)', fontSize: '0.75rem', fontWeight: '800' }}>DETAILS</th>
                            <th style={{ textAlign: 'left', padding: '1rem 1.5rem', color: 'var(--text-light)', fontSize: '0.75rem', fontWeight: '800' }}>STATUS</th>
                            <th style={{ textAlign: 'right', padding: '1rem 1.5rem', color: 'var(--text-light)', fontSize: '0.75rem', fontWeight: '800' }}>ACTION</th>
                        </tr>
                    </thead>
                    <tbody>
                        {payouts.length === 0 ? (
                            <tr><td colSpan="6" style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>No payout requests found.</td></tr>
                        ) : payouts.map(p => (
                            <tr key={p._id} style={{ borderBottom: '1px solid var(--border)' }}>
                                <td style={{ padding: '1rem 1.5rem', fontWeight: '700' }}>{p.username || p.userId}</td>
                                <td style={{ padding: '1rem 1.5rem', fontWeight: '900', color: '#10b981' }}>${(p.amount || 0).toFixed(2)}</td>
                                <td style={{ padding: '1rem 1.5rem', fontWeight: '700' }}>{p.method}</td>
                                <td style={{ padding: '1rem 1.5rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>{p.accountDetails}</td>
                                <td style={{ padding: '1rem 1.5rem' }}><StatusBadge status={p.status} /></td>
                                <td style={{ padding: '1rem 1.5rem', textAlign: 'right' }}>
                                    {p.status === 'Pending' && (
                                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                                            <button onClick={() => updateStatus(p._id, 'Complete')} style={{ background: '#10b981', color: 'white', border: 'none', padding: '0.4rem 0.75rem', borderRadius: '8px', cursor: 'pointer' }}><CheckCircle size={16} /></button>
                                            <button onClick={() => updateStatus(p._id, 'Cancelled')} style={{ background: '#ef4444', color: 'white', border: 'none', padding: '0.4rem 0.75rem', borderRadius: '8px', cursor: 'pointer' }}><XCircle size={16} /></button>
                                        </div>
                                    )}
                                </td>
                            </tr>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginBottom: '3rem' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  <button onClick={removeStep} style={{ background: 'var(--bg-alt)', border: '1px solid var(--border)', color: 'var(--text)', padding: '0.5rem 1rem', borderRadius: '8px', cursor: 'pointer' }}>-</button>
                  <span style={{ fontSize: '1.25rem', fontWeight: '950' }}>{config.steps} Steps</span>
                  <button onClick={addStep} style={{ background: 'var(--bg-alt)', border: '1px solid var(--border)', color: 'var(--text)', padding: '0.5rem 1rem', borderRadius: '8px', cursor: 'pointer' }}>+</button>
              </div>
            </div>
            <div>
              ...
              <input type="number" value={config.timer} onChange={e => setConfig({...config, timer: parseInt(e.target.value)})}
                style={{ width: '100%', padding: '0.875rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', fontWeight: '800' }} />
            </div>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                    <div>
                        ...
                        <textarea value={config.adCodes?.top} onChange={e => setConfig({...config, adCodes: {...config.adCodes, top: e.target.value}})} style={{ width: '100%', minHeight: '80px', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', fontSize: '0.8125rem', fontFamily: 'monospace' }} />
                    </div>
                    <div>
                        ...
                        <textarea value={config.adCodes?.sidebar} onChange={e => setConfig({...config, adCodes: {...config.adCodes, sidebar: e.target.value}})} style={{ width: '100%', minHeight: '80px', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', fontSize: '0.8125rem', fontFamily: 'monospace' }} />
                    </div>
                  </div>
                  <div>
                        ...
                        <textarea value={config.adCodes?.content} onChange={e => setConfig({...config, adCodes: {...config.adCodes, content: e.target.value}})} style={{ width: '100%', minHeight: '80px', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', fontSize: '0.8125rem', fontFamily: 'monospace' }} />
                  </div>
                   <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                    <div>
                        <label style={{ display: 'block', fontWeight: '800', fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>POPUNDER SCRIPT</label>
                        <textarea value={config.adCodes?.popunder} onChange={e => setConfig({...config, adCodes: {...config.adCodes, popunder: e.target.value}})} style={{ width: '100%', minHeight: '80px', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', fontSize: '0.8125rem', fontFamily: 'monospace' }} />
                    </div>
                    <div>
        ...
        <div style={{ background: 'var(--bg-card)', padding: '2rem', borderRadius: '24px', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    <button onClick={() => setEditing(null)} style={{ padding: '0.6rem 1.25rem', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', cursor: 'pointer' }}>Cancel</button>
            ...
            <input style={{ width: '100%', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', marginBottom: '1rem', fontWeight: '800' }} value={editing.title} onChange={e => setEditing({...editing, title: e.target.value})} />
            <textarea style={{ width: '100%', minHeight: '350px', padding: '1.25rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-alt)', color: 'var(--text)', fontSize: '0.9rem', fontFamily: 'monospace' }} value={editing.content} onChange={e => setEditing({...editing, content: e.target.value})} />
        </div>
    );
